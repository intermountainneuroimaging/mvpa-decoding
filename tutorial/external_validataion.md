...
python tutorial/pymvpa_style_validation.py \
    --our-results-dir out/haxby_object_classifier/1/test  # optional, prints this repo's own numbers alongside
```

(`test/` -- the independent-test-set "Full" family `mvpa_workflow.py` writes for
`config-haxby.example.json`'s matched train(1-9)/test(10-12) split; see
[tutorial/README.md](README.md#step-3-the-config).)
